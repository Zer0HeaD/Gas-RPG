...
bool URPG_ParkourMovementComponent::FSavedMove_ExtendedMoveCharacter::CanCombineWith(
...
void URPG_ParkourMovementComponent::FSavedMove_ExtendedMoveCharacter::Clear()
{
...
uint8 URPG_ParkourMovementComponent::FSavedMove_ExtendedMoveCharacter::GetCompressedFlags() const
{
...
void URPG_ParkourMovementComponent::FSavedMove_ExtendedMoveCharacter::SetMoveFor(
	ACharacter* C, float InDeltaTime, FVector const& NewAccel, FNetworkPredictionData_Client_Character& ClientData)
{
	FSavedMove_Character::SetMoveFor(C, InDeltaTime, NewAccel, ClientData);

	URPG_ParkourMovementComponent* CharacterMovement = Cast<URPG_ParkourMovementComponent>(C->GetCharacterMovement());
...
void URPG_ParkourMovementComponent::FSavedMove_ExtendedMoveCharacter::PrepMoveFor(ACharacter* C)
{
...
URPG_ParkourMovementComponent::FNetworkPredictionData_Client_ExtendedCharacter::
FNetworkPredictionData_Client_ExtendedCharacter(const UCharacterMovementComponent& ClientMovement) : Super(ClientMovement)
{

}

FSavedMovePtr URPG_ParkourMovementComponent::FNetworkPredictionData_Client_ExtendedCharacter::AllocateNewMove()
{
	return FSavedMovePtr(new FSavedMove_ExtendedMoveCharacter());
}

FNetworkPredictionData_Client* URPG_ParkourMovementComponent::GetPredictionData_Client() const
{
	check(PawnOwner != nullptr)

		if (ClientPredictionData == nullptr)
		{
			URPG_ParkourMovementComponent* MutableThis = const_cast<URPG_ParkourMovementComponent*>(this);
...
void URPG_ParkourMovementComponent::UpdateFromCompressedFlags(uint8 Flags)
{
...
void URPG_ParkourMovementComponent::OnMovementUpdated(float DeltaSeconds, const FVector& OldLocation, const FVector& OldVelocity)
{
...
bool URPG_ParkourMovementComponent::IsMovingOnGround() const
{
	return Super::IsMovingOnGround() || IsCustomMovementMode(CMOVE_Slide) || IsCustomMovementMode(CMOVE_Prone);
}

bool URPG_ParkourMovementComponent::CanCrouchInCurrentState() const
{
	return Super::CanCrouchInCurrentState() && IsMovingOnGround();
}

float URPG_ParkourMovementComponent::GetMaxSpeed() const
{
	Super::GetMaxSpeed();
...
float URPG_ParkourMovementComponent::GetMaxBrakingDeceleration() const
{
...
bool URPG_ParkourMovementComponent::CanAttemptJump() const
{
	return Super::CanAttemptJump() || IsWallrunning() || IsHanging() || IsClimbing();
}

bool URPG_ParkourMovementComponent::DoJump(bool bReplayingMoves, float DeltaTime)
{
...
void URPG_ParkourMovementComponent::UpdateCharacterStateBeforeMovement(float DeltaSeconds)
{
	//Slide
...
void URPG_ParkourMovementComponent::UpdateCharacterStateAfterMovement(float DeltaSeconds)
{
...
void URPG_ParkourMovementComponent::PhysCustom(float deltaTime, int32 Iterations)
{
...
void URPG_ParkourMovementComponent::OnMovementModeChanged(EMovementMode PreviousMovementMode, uint8 PreviousCustomMode)
{
...
//CONSTRUCTOR
URPG_ParkourMovementComponent::URPG_ParkourMovementComponent()
{
	NavAgentProps.bCanCrouch = true;
	bOrientRotationToMovement = false;
}

void URPG_ParkourMovementComponent::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	Super::GetLifetimeReplicatedProps(OutLifetimeProps);

	DOREPLIFETIME_CONDITION(URPG_ParkourMovementComponent, Proxy_bDashStart, COND_SkipOwner);

	DOREPLIFETIME_CONDITION(URPG_ParkourMovementComponent, Proxy_bShortMantle, COND_SkipOwner);
	DOREPLIFETIME_CONDITION(URPG_ParkourMovementComponent, Proxy_bTallMantle, COND_SkipOwner);
}

void URPG_ParkourMovementComponent::InitializeComponent()
{
	Super::InitializeComponent();

	ExtendedCharacterOwner = Cast<ARPG_Player_Character>(GetOwner());
}

void URPG_ParkourMovementComponent::Server_EnterProne_Implementation()
{
	Safe_bWantsToProne = true;
	//CharacterOwner->bUseControllerRotationYaw = false;
}

void URPG_ParkourMovementComponent::EnterProne(EMovementMode PrevMode, ECustomMovementMode PrevCustomMode)
{
	bWantsToCrouch = true;
...
void URPG_ParkourMovementComponent::ExitProne()
{
	bWantsToCrouch = false;
	//bOrientRotationToMovement = false;
	//CharacterOwner->bUseControllerRotationYaw = true;
}
bool URPG_ParkourMovementComponent::CanProne() const
{
	return	IsCustomMovementMode(CMOVE_Prone) ||
		(IsMovementMode(MOVE_Walking) && IsCrouching());
}
void URPG_ParkourMovementComponent::PhysProne(float deltaTime, int32 Iteration)
{
...
void URPG_ParkourMovementComponent::EnterSlide(EMovementMode PrevMode, ECustomMovementMode PrevCustomMode)
{
	bWantsToCrouch = true;
...
void URPG_ParkourMovementComponent::ExitSlide()
{
	bWantsToCrouch = false;
}
bool URPG_ParkourMovementComponent::CanSlide() const
{
	FVector Start = UpdatedComponent->GetComponentLocation();
...
void URPG_ParkourMovementComponent::PhysSlide(float deltaTime, int32 Iterations)
{
	if (deltaTime < MIN_TICK_TIME)
	{
		return;
...
bool URPG_ParkourMovementComponent::GetSlideSurface(FHitResult& Hit) const
{
	FVector Start = UpdatedComponent->GetComponentLocation();
...
void URPG_ParkourMovementComponent::OnDashCooldownFinished()
{
	Safe_bWantsToDash = true;
}
bool URPG_ParkourMovementComponent::CanDash() const
{
	//IsWalking() &&
	return IsWalking() && !IsCrouching() || IsFalling();
}
void URPG_ParkourMovementComponent::PerformDash(bool bDolphinDive)
{
...
void URPG_ParkourMovementComponent::OnRep_DashStart()
{
	DashStartDelegate.Broadcast();
}

void URPG_ParkourMovementComponent::OnRep_ShortMantle()
{
	CharacterOwner->PlayAnimMontage(ProxyShortMantleMontage);
}
void URPG_ParkourMovementComponent::OnRep_TallMantle()
{
	CharacterOwner->PlayAnimMontage(ProxyTallMantleMontage);
}
bool URPG_ParkourMovementComponent::TryMantle()
{
	//bWantsToCrouch = false;
...
bool URPG_ParkourMovementComponent::IsServer() const
{
	return CharacterOwner->HasAuthority();
}
float URPG_ParkourMovementComponent::CapR() const
{
	return CharacterOwner->GetCapsuleComponent()->GetScaledCapsuleRadius();
}
float URPG_ParkourMovementComponent::CapHH() const
{
	return CharacterOwner->GetCapsuleComponent()->GetScaledCapsuleHalfHeight();
}


bool URPG_ParkourMovementComponent::TryWallrun()
{
	...
	GetWorld()->GetTimerManager().SetTimer(
		...
		this,
		&URPG_ParkourMovementComponent::OnWallrunArchApplied,
		ArchFactorAppliedDelay);
	SLOG("Starting WallRun")
		return true;
}
void URPG_ParkourMovementComponent::PhysWallrun(float deltaTime, int32 Iterations)
{
	if (deltaTime < MIN_TICK_TIME)
	{
		return;
...
void URPG_ParkourMovementComponent::OnWallrunArchApplied()
{
	bStartWallrunArch = true;
}

void URPG_ParkourMovementComponent::ResetWallrunArch()
{
	bStartWallrunArch = false;
	GetWorld()->GetTimerManager().ClearTimer(TimerHandle_ArchWallrun);
}

bool URPG_ParkourMovementComponent::TryHang()
{
...
bool URPG_ParkourMovementComponent::TryClimb()
{
...
void URPG_ParkourMovementComponent::PhysClimb(float deltaTime, int32 Iterations)
{
	if (deltaTime < MIN_TICK_TIME)
	{
		return;
...
bool URPG_ParkourMovementComponent::IsCustomMovementMode(ECustomMovementMode InCustomMovementMode) const
{
	return MovementMode == MOVE_Custom && CustomMovementMode == InCustomMovementMode;
}
bool URPG_ParkourMovementComponent::IsMovementMode(EMovementMode InMovementMode) const
{
	return MovementMode == InMovementMode;
}

void URPG_ParkourMovementComponent::SprintPressed()
{
	Safe_bWantsToSprint = true;
}
void URPG_ParkourMovementComponent::SprintReleased()
{
...
void URPG_ParkourMovementComponent::DashPressed()
{
	...
	else
	{
		GetWorld()->GetTimerManager().SetTimer(
			...
			this,
			&URPG_ParkourMovementComponent::OnDashCooldownFinished,
			DashCoolDownDuration);
	}
}
void URPG_ParkourMovementComponent::DashReleased()
{
	GetWorld()->GetTimerManager().ClearTimer(TimerHandle_DashCooldown);
	Safe_bWantsToDash = false;
}

void URPG_ParkourMovementComponent::ClimbPressed()
{
	TryClimb();
	//if (IsFalling() || IsClimbing() || IsHanging()) bWantsToCrouch = true;
}

void URPG_ParkourMovementComponent::ClimbReleased()
{
	bWantsToCrouch = false;
}

void URPG_ParkourMovementComponent::PronePressed()
{
	bWantsToCrouch = ~bWantsToCrouch;
	GetWorld()->GetTimerManager().SetTimer(
		TimerHandle_EnterProne,
		this,
		&URPG_ParkourMovementComponent::TryEnterProne,
		Prone_EnterHoldDuration);
}

void URPG_ParkourMovementComponent::ProneReleased()
{
	GetWorld()->GetTimerManager().ClearTimer(TimerHandle_EnterProne);
}

void URPG_ParkourMovementComponent::DolphinDivePressed()
{
	// apply custom params
	GravityScale = 2.f;
