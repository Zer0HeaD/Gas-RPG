...
#include "Character/ProneAnimInstance.h"
...
void UProneAnimInstance::NativeUpdateAnimation(float DeltaSeconds)
{
	...
	if (!Player)
	{
		Player = Cast<ARPG_Player_Character>(TryGetPawnOwner());
	}
	...
	if (!MovementComponentRef)
	{
		if (Player)
		{
			MovementComponentRef = Cast<URPG_ParkourMovementComponent>(Player->GetMovementComponent());
		}
	}

	if (Player) 
	{
		IsProne = MovementComponentRef->IsProning();
...
void UProneAnimInstance::TurnBodyYaw(float& Yaw, float& ProneYaw)
{
	...
	if (MovementComponentRef->IsProning())
	{
